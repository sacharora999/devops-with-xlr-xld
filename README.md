...
Get the workshop
Download and extract the workshop zip:
...
The workshop requires you to bring your own licenses for XL Deploy and XL Release. You can use production licenses or request trial licenses for XL Deploy and XL Release.

Copy the XL Deploy license to docker/xl-deploy/default-conf/deployit-license.lic

Copy the XL Release license to docker/xl-release/default-conf/xl-release-license.lic

Start up the XL DevOps Platform
If you are already running XL Deploy or XL Release on your local machine, please stop them.

If you are running the workshop on Windows, execute the following command to be able to run the Docker Compose file:

> set COMPOSE_CONVERT_WINDOWS_PATHS=1
For more information on this environment variable, read the documentation for Docker Compose
...
Open the XL Deploy GUI at http://localhost:4516/ and login with the username admin and password admin. Verify that the about box reports the version to be 8.6.1.

Open the XL Release GUI at http://localhost:5516/ and login with the username admin and password admin. Verify that the about box reports the version to be 8.6.1.
...
Test the XL CLI by executing the following command:
$ xl help
The output should look like this:
...
Open the XL Deploy GUI, find the local-docker entry in the Infrastructure tree and run the Check Connection control task.
...
Open a new terminal window and go to the directory where you unzipped the devops-as-code-workshop repository.
...
Import the REST-o-rant-api package:
$ xl apply -f exercise-3/rest-o-rant-api-docker.yaml
Open the XL Deploy GUI and review version 1.0 of the rest-o-rant-api-docker application. Compare it with the contents of the XL YAML file that you applied in the previous step.

Using the GUI, deploy version 1.0 of the rest-o-rant-api-docker application to the Local Docker Engine environment.

When the deployment has finished, check whether the rest-o-rant-api container is running:
...
Import the new version of the rest-o-rant-api-docker package, as well as the rest-o-rant-web-docker package:
$ xl apply -f exercise-4/rest-o-rant-docker.yaml
Open the XL Deploy GUI and review the two packages that you just imported and compare them with the XL YAML file. Notice the net rest-o-rant-network deployable in the rest-o-rant-api-docker package.

Using the GUI, deploy version 1.1 of the rest-o-rant-api-docker package to the Local Docker Engine environment.

Using the GUI, deploy version 1.0 of the rest-o-rant-web-docker package to the Local Docker Engine environment.

When the deployment has finished, open a new browser tab and access it at http://localhost:8181/. You should see a text saying "Find the best restaurants near you!". Type "Cow" in the search bar and click "Search" to find the "Old Red Cow" restaurant.
...
Undeploy the rest-o-rant-web application from the Local Docker Engine environment.

Undeploy the rest-o-rant-api application from the Local Docker Engine environment.

But let's make sure that you don't forget next time that you run this workshop. Let's create a simple pipeline.

Import that REST-o-rant pipeline:
$ xl apply -f exercise-5/rest-o-rant-docker-pipeline.yaml
Open the XL Release GUI, go to Design tab, click on the REST-o-rant folder and then go to the Templates tab. Review the REST-o-rant on Docker pipeline that you've just imported and compare it to the XL YAML file.

Start a new release from that template and follow the instructions.

Exercise 6: Simplify the package and the pipeline by using dependencies
You'll have noticed that the pipeline has two separate steps to deploy and later undeploy the rest-o-rant-api-docker and the rest-o-rant-web-docker packages. If we make the frontend depend on the backend, we can simplify the pipeline and ensure that the frontend is not deployed without the backend.
...
In XL Deploy, open Applications/rest-o-rant-web-docker/1.0 and add the following entry to the Application Dependencies map:
Key	Value
rest-o-rant-api-docker	1.1
and change Undeploy Unused Dependencies to true.

In XL Release, from the REST-o-rant on Docker pipeline template, remove the Deploy REST-o-rant application backend task from the first phase and the Undeploy REST-o-rant application backend step from the last phase.

Run the pipeline to verify that the application dependencies and the updated pipeline function correctly. When the pipeline has completed, go to the XL Deploy UI and check that the rest-o-rant-api-docker application was undeployed.

Generate the XL YAML files for the changes:
...
Review the generated XL YAML files and compare them with the originals from exercises 4 and 5 respectively.
...
In the XL Deploy UI, import the PetClinic-ear/1.0 sample package from the XL Deploy server.

Generate the XL YAML file for the package you just imported to see how to specify artifacts:

$ xl generate -s xl-deploy -p Applications/PetClinic-ear/1.0 -f exercise-7/petlinic-ear.yaml
Review the generated XL YAML file and the artifact.
...
Install the AWS CLI as described here: https://docs.aws.amazon.com/cli/latest/userguide/installing.html

Configure the AWS CLI as described here and select region eu-west-1 (Ireland): https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-getting-started.html#cli-quick-configuration
...
If you do not have Python installed on your machine, install it.

Run the provided Python script to generate an XL YAML file with your credentials:

$ ./exercise-8/awsconfig2xld.py > exercise-8/AWSConfig.yaml
Review the generate XL YAML file.

Apply the generated XL YAML file:

$ xl apply -f exercise-8/AWSConfig.yaml
In XL Deploy, navigate to the Infrastructure/aws object in the Explorer tree and execute the "Check connection" task to verify that the connection could be made.
...
Import the CloudFormation template and its metadata into XL Deploy:
xl apply -f exercise-9/ecommerce-infrastructure.yaml 
In XL Deploy, deploy version 1.0 of the ecommerce-infrastructure package to the AWS environment.

When the deployment has finished, go to the AWS Console, navigate to the CloudFormation service, and review the ecommerce-cloudformation stack and the resources it has created:
...
Import the e-commerce application into XL Deploy:
$ xl apply -f exercise-10/ecommerce-application.yaml 
Import the e-commerce pipeline into XL Release:
$ xl apply -f exercise-10/ecommerce-pipeline.yaml
In XL Release, create a new release based on the e-Commerce CD pipeline template in the e-Commerce folder. Do not start it yet!
...
When the release gets to the Verify application manually task, open the task and follow the instructions to complete it.

Wait until the release has finished.

Exercise 11: Shutting down the XL DevOps Platform
Shut down the XL DevOps Platform:
$ docker-compose down
